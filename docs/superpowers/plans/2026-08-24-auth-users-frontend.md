...
The `hasRequested` ref guard is required, not defensive boilerplate: React 19 Strict Mode double-invokes effects in development, and this effect's request isn't idempotent (the token is deleted server-side after its first successful use). Without the guard, the second invocation's failure overwrites the first invocation's success state. curl-only verification cannot catch this — see the Global Constraints note.
