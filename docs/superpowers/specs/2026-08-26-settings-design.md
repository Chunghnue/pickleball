...
- **Bookings, tạo booking mới** (`bookings.service.ts`) — **chỉ nhánh `create()`** (customer tự đặt) đã có call `notificationsService.notifyNewBookingForOwner({ to: owner.email, ... })` không điều kiện; sửa: bọc trong `if (notificationSettings.newBooking)`, đổi `to` từ `owner?.email` sang `venue.email ?? owner?.email`. **`createForOwner()`** (owner đặt hộ qua module Customers) **không** gọi và **tiếp tục không gọi** — người tạo booking chính là owner, tự thông báo cho hành động của mình là nhiễu, đúng lý do đã áp dụng cho huỷ lịch bên dưới (không gửi khi `cancelledBy` là chính owner).
