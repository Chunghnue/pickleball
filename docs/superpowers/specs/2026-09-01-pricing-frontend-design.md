...
3. Nếu `venueId` suy ra được vẫn là `ALL_BRANCHES_ID` (chưa chọn courtId và chưa chọn chi nhánh cụ thể): fallback **chỉ trong state riêng của trang** về chi nhánh đầu tiên của owner (`GET /api/venues/mine`, lấy phần tử đầu) — **không** gọi `setSelectedVenueId` cho trường hợp này, và **không** chặn trang bằng empty state yêu cầu chọn chi nhánh. Đúng theo cách trang Đặt lịch (`/owner/bookings`) đã xử lý: đọc `selectedVenueId` từ switcher nhưng không ghi fallback ngược lại switcher, để tránh việc vào trang Bảng giá lại âm thầm đổi dropdown "CHI NHÁNH" toàn cục cho cả các trang khác. Chỉ hiện empty state thật sự khi owner chưa có chi nhánh nào (`venues.length === 0`).
