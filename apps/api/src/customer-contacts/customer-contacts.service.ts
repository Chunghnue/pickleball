...
export class CustomerContactsService {
  ...
  async resolveSelector(
    ...
    if (selector.customerId) {
      ...
      if (!user) {
        throw new NotFoundException(
          `Khách hàng ${selector.customerId} không tồn tại`,
        );
      }
    ...
    if (selector.customerContactId) {
      const contact = await this.findByIdForOwner(
        ownerId,
        selector.customerContactId,
      );
  ...
  async create(
    ...
    if (existing) {
      throw new ConflictException(
        `Đã tồn tại khách hàng với số điện thoại ${data.phone}`,
      );
    }
    try {
      return await this.repository.save(
