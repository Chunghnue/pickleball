...
describe('Payments owner-notification gating (e2e)', () => {
  ...
  it('emails the owner on mark-paid when the setting is on', async () => {
    const owner = await createUser(dataSource, 'payowner-notif@test.com', UserRole.OWNER);
    const venue = await createVenue(dataSource, owner.id, 'Venue A');
    const court = await createCourt(dataSource, venue.id, 'Sân 1');
    await createUser(dataSource, 'paycustomer-notif@test.com', UserRole.CUSTOMER);
    ...
    const createResponse = await request(app.getHttpServer())
      ...
      .send({ courtId: court.id, date: '2099-06-01', startTime: '08:00', endTime: '09:00' })
      .expect(201);

    mockMailService.send.mockClear();

    await request(app.getHttpServer())
      .post(`/venues/mine/${venue.id}/bookings/${createResponse.body.id}/payment/mark-paid`)
